...
char * spp_process_clienthello(SSL *s) {
    int i;
    ...
    for (;;) {
        ...
        switch (s->state) {
            ...
            case SSL3_ST_SR_CLNT_HELLO_C:
                ...
                s->other_ssl = next_st;
                next_st->other_ssl = s;
                if (next_st->handshake_func == 0)
                    /* Not properly initialized yet */
                    SSL_set_connect_state(next_st);
            ...
            case SSL3_ST_CR_SRVR_HELLO_B:
                ret=ssl3_get_server_hello(next_st);
                printf("Received server hello\n");
		if (ret <= 0) goto end;
                
                ret=spp_forward_message(s, next_st);
                if (ret <= 0) goto end;
            ...
            case SSL3_ST_CR_CERT_B:
                ret=ssl3_get_server_certificate(next_st);
            ...
            case SSL3_ST_CR_KEY_EXCH_B:                
                ret=ssl3_get_key_exchange(next_st);
            ...
            case SSL3_ST_CR_SRVR_DONE_B:
                ret=ssl3_get_server_done(next_st);
                ...
                if (s->s3->tmp.cert_req)
                    ...
                    s->state=SPP_ST_CW_PRXY_CERT_A;
                }
                s->init_num=next_st->init_num=0;

                break;
                
            case SPP_ST_CW_PRXY_CERT_A:
            case SPP_ST_CW_PRXY_CERT_B:
                ...
                s->state=SPP_ST_CW_PRXY_KEY_EXCH_A;
                s->init_num=next_st->init_num=0;
                break;

            case SPP_ST_CW_PRXY_KEY_EXCH_A:
            case SPP_ST_CW_PRXY_KEY_EXCH_B:
                ...
                s->state=SPP_ST_CW_PRXY_DONE_A;
                s->init_num=next_st->init_num=0;
                break;
                
            case SPP_ST_CW_PRXY_DONE_A:
            case SPP_ST_CW_PRXY_DONE_B:
